...
final public class FastClonerFactory {
    ...
    public FastCloner getFastCloner(Class<?> c, ClassLoader cl) {
        var key = c.hashCode() + "_" + cl.hashCode();
    ...
    private FastCloner genCloner(String key, Class<?> c, ClassLoader cl) {
        var fcMetaInfo = fcMetaInfos.computeIfAbsent(c, this::generateClonerMetaInfo);

        var fcClz = cachedFastClonerClasses.computeIfAbsent(key,
    ...
    private void serializeConfigs(ClassWriter cw, String className, CloneConfig cc) {
        ...
        for (var configKey : configs.keySet()) {
            var confValue = configs.get(configKey);
    ...
    private void visitCloneMethod(ClassWriter cw, Class<?> clz) {
        ...
        //~~~ set fields
        var visitedFieldNames = new HashSet<>();
    ...
    private void handleFinalFieldSet(MethodVisitor mv, Field f) {
        if (f.getType().isPrimitive()) {
            var methodName = "set" + StringUtil.capFirst(f.getType().getName());
            var methodDesc = "(Ljava/lang/Object;J" + getDescriptor(f.getType()) + ")V";
